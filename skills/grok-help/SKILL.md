...
1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `${HOME}/.grok/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `${HOME}/.grok/docs/user-guide/`. The available guides are:
   ...
   `${HOME}/.grok/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `${HOME}/.grok/config.toml` with search_replace.

4. To **create a skill** for the user, create `${HOME}/.grok/skills/<name>/SKILL.md`
   (read `${HOME}/.grok/docs/user-guide/08-skills.md` for the SKILL.md format).
