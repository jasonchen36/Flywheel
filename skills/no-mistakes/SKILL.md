...
- **Run `git fetch origin --quiet && git status -sb` before trusting any local
  claim about branch state, merge status, or what's already on the remote.**
  A stale local checkout (not fetched recently) can make you assert something
  is unmerged/missing/behind when origin has already moved — or vice versa.
  Verify against the fetched remote before stating branch/merge state as fact.
