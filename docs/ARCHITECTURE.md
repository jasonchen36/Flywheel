...
## ACE, rollout, and self-harness lifecycle

`ace_reflector.py` treats its optional provider as advisory and tolerates malformed lessons, bounded scans, and provider failures without blocking the pipeline. `ace_playbook.py` normalizes score and lesson state, validates command bounds, ranks and deduplicates bullets deterministically, and replaces coordinated outputs atomically. `agent_rollouts.py` validates every scenario, transcript identity, rubric expectation, and baseline. A dry run cannot mutate baseline, history, or latest state; an existing non-clean baseline requires explicit confirmation; malformed or unavailable provider output is a documented skip rather than a false behavior regression.

`self_harness.py` bounds child evaluation commands, converts spawn and timeout failures into structured diagnostics, archives negative results under one deduplicated transaction, and evaluates held-out plus rollout acceptance before any ACE rebuild. Its atomic latest report records an explicit `reported`, `accepted`, `rejected`, `applied`, or `apply_failed` outcome; healthcheck treats rejected and apply-failed cycles as errors. `consolidate_memory.py` shares the memory-index lock, excludes symlinks, creates collision-safe backups, and restores live memory if a commit fails. `chronic_failures.py` is diagnostic-only: it rotates repeated blocked regressions toward the next untried intervention and writes at most one snapshot per pattern per UTC day. `intent_how_audit.py` is likewise diagnostic-only, scans regular Markdown skills without following symlinks, protects safety constraints from deprecation advice, and writes bounded atomic reports.

## SessionEnd observability

The SessionEnd hook remains non-blocking, but each stage writes a dedicated log and appends a five-column status row—timestamp, stage, status, exit code, and duration in milliseconds—under `MEMORY/LEARNING/DIAGNOSTICS/session-end/`. Atomic `latest.json` summarizes total duration, stage count, failure count, skipped dependent stages, and failed stage names. `skipped.json` records overlapping invocations without replacing the active run summary. Held-out, rollout, and self-harness prerequisites explicitly gate ACE rebuilding and memory consolidation: a failed prerequisite records those mutations as skipped rather than allowing learning state to advance. The native lock uses `flock` when available and an atomic directory lock with stale-PID recovery otherwise.

## Quality thresholds

The complete test command enforces a 93% branch-aware repository coverage floor, up from the original baseline of approximately 11%. Twenty-five critical modules independently require 100% statement and branch coverage: shared durable state, transactional review, validated configuration, surface permissions, ratings hygiene, config-only enforcement promotion, shared summary ingestion, lesson deduplication, lesson evolution, pattern promotion, strict outcome judging, the binary evaluation registry, effectiveness measurement, held-out regression decisions, changelog integrity, the self-improvement engine, skill autofix, skill burn-in, ACE reflection, ACE playbook curation, live rollout evaluation, self-harness orchestration, memory consolidation, chronic-failure rotation, and the intent audit. Direct lifecycle suites additionally cover held-out fixtures, SessionEnd dependency gating, session auto-seeding, graph synchronization, lesson retirement, exact-turn pattern promotion, Git-backed autofix confirmation and rollback, and the end-to-end judge-to-effectiveness-to-review flow; healthcheck's operational decision paths remain regression-tested across healthy, stale, corrupt, missing, rejected, and degraded installations.
