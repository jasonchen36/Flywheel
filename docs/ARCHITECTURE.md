...
The complete test command enforces a 39% branch-aware repository coverage floor, up from the original baseline of approximately 11%. Shared durable state, transactional review, validated configuration, surface permissions, ratings hygiene, and config-only enforcement promotion independently require 100% statement and branch coverage. Direct lifecycle suites cover held-out fixtures, self-harness orchestration, session auto-seeding, and graph synchronization; healthcheck's operational decision paths remain regression-tested across healthy, stale, corrupt, missing, and degraded installations.
