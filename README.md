...
`harness_healthcheck.py --json` reports malformed review rows, unknown statuses, processing claims, failed actions, invalid enforcement configuration, unresolved autofix rollback failures, audit-failed edits, quarantined autofix ledger records, held-out or rollout gate failures, invalid rollout baselines, malformed ACE bullets, weak ACE output, unreadable self-harness lessons, and rejected or failed self-harness apply outcomes. Outcome judging treats empty or malformed provider output as unavailable and leaves the turn queued for retry; malformed queue rows are moved to `SIGNALS/invalid_judge.jsonl` so they remain inspectable without blocking later work. Valid judge results, queue draining, and result deduplication share one transaction boundary.

`self_improve.py` applies durable reclassification by exact `session_id|timestamp` identity, validates structured lesson and eval-candidate schemas, and fails closed on unknown background provider names or missing cloud projects. Candidate, reclassification, lesson, and index mutations are semantically deduplicated or lock-scoped; rejected lesson content cannot be reported or indexed as written. Pattern promotion retains exact turns for evidence while counting distinct sessions toward promotion thresholds. Binary eval results use the same exact-turn identity, validate captured IDs and patterns against the code catalog, and reconcile result plus registry state under one lock while preserving legacy timestamp rows. Lesson evolution creates stable candidate batches, pauses while review is pending, applies only the latest batch, supersedes stale alternatives, and restores the live lesson if its ledger commit fails.

Skill autofix now treats snapshots and validations as fail-closed transaction boundaries. Missing or malformed gates block new mutations; rollback failures, audit failures, and invalid active state remain explicit ledger statuses rather than optimistic success. Guardrail apply, restore, Git snapshots, burn-in transitions, and ledger persistence share coordinated locks, with Git-backed end-to-end tests covering both confirmation and automatic rollback.

`harness_changelog.py` fingerprints watched files with SHA-256, so same-size edits are detected even when timestamps are preserved. It excludes external symlinks, transient atomic-write files, and portable lock-owner directories while accepting and upgrading older size-and-mtime snapshots. ACE reflection and curation validate malformed lesson, score, and provider state before atomic output replacement. Live rollout scenarios and baselines are schema-validated; dry runs do not mutate history, baselines, or latest state, and non-clean baselines require explicit confirmation. Self-harness validation children have bounded execution, final outcomes are written to a stable atomic report, and SessionEnd will not rebuild ACE or consolidate memory unless held-out and rollout prerequisites pass. Memory consolidation shares the lesson-index lock, creates collision-safe backups, and restores the original index after a failed write. The intent-versus-HOW audit scans multiline recipes without following symlinks and remains diagnostic-only.
...
The gate runs Ruff, mypy with Python 3.10 compatibility, Python compilation, the complete branch-aware pytest suite with a 93% repository non-regression floor, a Bun build of all hooks and pi extensions, Bandit, Python and JavaScript dependency audits, and ShellCheck. Twenty-five critical modules independently require 100% statement and branch coverage: durable state I/O, transactional review storage, validated configuration, surface permissions, ratings hygiene, config-only enforcement promotion, shared summary ingestion, lesson deduplication, lesson evolution, pattern promotion, strict outcome judging, the binary evaluation registry, effectiveness measurement, held-out regression decisions, hashed changelog integrity, the self-improvement engine, skill autofix, skill burn-in, ACE reflection, ACE playbook curation, live rollout evaluation, self-harness orchestration, memory consolidation, chronic-failure rotation, and the intent-versus-HOW audit. Direct lifecycle tests additionally cover held-out fixture validation, SessionEnd dependency gating, session auto-seeding, graph synchronization, lesson retirement, exact-turn pattern promotion, Git-backed autofix confirmation and rollback, and the judge-to-effectiveness-to-human-review flow without live network or model dependencies. Individual targets include `make test`, `make coverage-foundations`, `make lint`, `make hooks`, `make security`, and `make shellcheck`.
