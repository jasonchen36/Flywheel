...
| `make coverage-foundations` | Enforce 100% statement and branch coverage for eighteen critical state, review, configuration, permission, hygiene, ingestion, promotion, judging, evaluation, effectiveness, held-out, changelog, self-improvement, evolution, autofix, and burn-in modules. |
...
All installed runtime paths must derive from `learning/harness_paths.py` or the equivalent `HARNESS_HOME` resolution in TypeScript. Do not introduce new hard-coded `~/.claude` state paths. Whole-file mutable state, generated lessons and reports, and single or batched JSONL writes should use `learning/state_io.py`; do not add direct mutable-state writes. TypeScript appenders for files that Python can rewrite must use `runtime/state-io.ts`, preserve the token-and-PID ownership protocol, and remain covered by cross-process Python/Bun tests. Shared review mutations must use `learning/review_store.py`; never implement a separate load-modify-rewrite cycle for `pending_human_review.jsonl`. Meeting and scrum wrappers must delegate candidate discovery, transactional ledger commits, and portable flushing to `learning/summary_ingest.py` rather than copying ingestion logic. Enforcement configuration changes must pass through `learning/harness_config.py` and keep Claude/pi normalization keys synchronized. Judge-provider responses must be schema-validated: a valid empty failure list is distinct from unavailable or malformed output, and result append plus queue drain must stay deduplicated and retry-safe. Held-out regression producers must rely on review-store semantic deduplication rather than stale queue snapshots. Changelog snapshots must remain content-hashed and must not follow external symlinks or observe lock and temporary-file artifacts. Self-improvement provider names must resolve through an explicit allowlist, structured lesson and eval-candidate output must be schema-normalized before mutation, and durable classification must use exact turn identities rather than session-only keys. Eval candidates and reclassification records must remain semantically deduplicated; lesson and memory-index updates must remain lock-scoped and must not report success when validation rejects a write. New critical branches require complete branch coverage, including the self-improvement engine, outcome judging, the binary eval registry, effectiveness measurement, held-out regression, changelog integrity, lesson evolution, skill autofix, and skill burn-in in addition to the existing state, review, configuration, surface, ratings, promotion, ingestion, deduplication, and taxonomy foundations. Evaluation captures must validate code-owned IDs and patterns, persist exact turn identities, preserve legacy joins, and reconcile results plus registry state under one lock. Lesson-evolution candidates must carry stable proposal and batch identities; approvals may apply only the latest pending batch, must supersede stale alternatives, and must restore the live lesson if ledger persistence fails. Autofix snapshots, live skill writes, validations, restores, burn-in transitions, and ledger writes must remain lock-coordinated and fail closed; never record a revert, apply, or audit step as successful when its underlying mutation failed. Lesson identifiers used for file mutation must be validated, backup names must never overwrite prior evidence, and live lesson or taxonomy edits must use atomic replacement. Keep generated validation commands bounded: untrusted or model-authored text must never be passed through a shell.
