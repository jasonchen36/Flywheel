...
| `make coverage-foundations` | Enforce 100% branch coverage for shared state-I/O primitives. |
...
All installed runtime paths must derive from `learning/harness_paths.py` or the equivalent `HARNESS_HOME` resolution in TypeScript. Do not introduce new hard-coded `~/.claude` state paths. Whole-file state rewrites and JSONL appends should use `learning/state_io.py`; new foundation branches require complete branch coverage. Keep generated validation commands bounded: untrusted or model-authored text must never be passed through a shell.

The installer must remain idempotent. Existing user-owned state and configuration should not be overwritten unless the behavior is explicitly documented, backed up, and tested. SessionEnd must remain non-blocking and single-run per installation; stage dependencies should be explicit and sequential unless shared-state independence is proven. Failures must remain observable through status or diagnostic output.
